<template>
  <div class="pnbi-datatable dark">
    <v-toolbar flat dark color="accent" class="pnbi-datatable__toolbar">
      <h3 :class="{'title': title.length, 'headline': headline.length}">{{label}}</h3>
        <v-spacer></v-spacer>
        <slot name="controls">
        </slot>
        <v-flex xs3>
          <v-text-field clearable class="pnbi-datatable__search"
            solo-inverted flat v-model="search" label="Suche…"
            append-icon="search"
          ></v-text-field>
        </v-flex>
        <v-btn small color="primary" dark @click="$emit('new')">
          <v-icon class="mr-2" dark left>add</v-icon>
          {{buttonLabel}}
        </v-btn>
    </v-toolbar>
    <slot name="datatable">
    </slot>
  </div>
</template>

<script>
export default {
  props: {
    title: {
      type: String | null,
      default: ''
    },
    headline: {
      type: String | null,
      default: ''
    },
    /*     type: {
      type: String | null,
      default: 'full'
    }, */
    buttonLabel: {
      type: String | null,
      default: 'Neu'
    }
  },
  watch: {
    search (newValue) {
      this.$emit('search', newValue)
    }
  },
  computed: {
    label () {
      return this.title.length ? this.title : this.headline
    }
  },
  data () {
    return {
      search: null
    }
  }
}
</script>

<style lang="css" scoped>
div >>> .pnbi-datatable__toolbar .v-toolbar__content {
  height: 54px !important;
}
div >>> .v-input.pnbi-datatable__search {
  height: 30px !important;
  min-height: 30px !important;
}
div >>> .v-input.pnbi-datatable__search .v-label {
  font-size: 14px;
}
div >>> .v-input.pnbi-datatable__search .v-input__control {
  min-height: 30px;
}
.pnbi-datatable >>> thead tr {
  height: 28px;
}
.pnbi-datatable >>> thead tr th {
  font-weight: 600;
}

.pnbi-datatable .v-alert.warning {
  margin-top: 0px;
  margin-left: -24px;
  margin-right: -24px;
}
.pnbi-datatable.dark {
}
/*shared*/
.pnbi-datatable.dark >>> thead {
  background-color: #3f515d;
  color: #fff;
}

.pnbi-datatable.dark >>> thead tr th {
  color: rgba(255, 255, 255, 0.8);
}

.pnbi-datatable.dark >>> tbody tr:hover {
  /*background: #ccd4da !important;*/
  background: rgba(204, 212, 218, 0.5) !important;
}

.pnbi-datatable.dark >>> .v-datatable thead tr th.column.active,
.pnbi-datatable.dark >>> .v-datatable thead tr th.column:hover,
.pnbi-datatable.dark >>> .v-datatable thead tr th.column.active:hover,
.pnbi-datatable.dark >>> .v-datatable thead tr th.column.active.sortable:hover,
.pnbi-datatable.dark >>> .v-datatable thead tr th.column .v-icon,
.pnbi-datatable.dark >>> .v-datatable thead tr th.column.active .v-icon {
  color: rgba(255, 255, 255, 0.9) !important;
}

/*light theme*/
.pnbi-datatable.dark {
  background-color: #3f515d;
}
.pnbi-datatable.dark tr.active {
  background: rgba(204, 212, 218, 0.6) !important;
}
.pnbi-datatable.dark tr.active + tr.v-datatable__expand-row .v-card {
  background-color: rgba(204, 212, 218, 0.6) !important;
}
/* .pnbi-child-table {
  xxbackground-color: rgba(204, 212, 218, 0.6) !important;
}
.pnbi-child-table .table {
  background-color: rgba(204, 212, 218, 0.6) !important;
} */
</style>
